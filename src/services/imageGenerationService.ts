...
interface ImageInferenceMessage {
  taskType: string;
  taskUUID: string;
  model: string;
  width: number;
...
export class RunwareService {
  ...
  async generateImage(params: GenerateImageParams): Promise<GeneratedImage> {
    ...
    return new Promise((resolve, reject) => {
      const message: ImageInferenceMessage = {
      ...
      if (params.seed) {
        message.seed = params.seed;
      }

      if (message.model !== "runware:100@1" && params.promptWeighting) {
        message.promptWeighting = params.promptWeighting;
      }
